...
HRESULT Scene::InitializeShaders( ID3D11Device* device )
{
	...
	if ( SUCCEEDED( hr = CompileShader( "Code/Shader/Shader.hlsl", "VS_main", "vs_5_0", nullptr, &pvs ) ) )
    {
                                                          ...
                                                          &mVertexShader ) ) )
        {
			D3D11_INPUT_ELEMENT_DESC inputDesc[] = {                
				{ "XPOSITION", 0, DXGI_FORMAT_R32_FLOAT, 0, 0, D3D11_INPUT_PER_VERTEX_DATA, 0 },
				{ "YPOSITION", 0, DXGI_FORMAT_R32_FLOAT, 1, 0, D3D11_INPUT_PER_VERTEX_DATA, 0 },
				{ "ZPOSITION", 0, DXGI_FORMAT_R32_FLOAT, 2, 0, D3D11_INPUT_PER_VERTEX_DATA, 0 }
			};
    ...
    if( SUCCEEDED( hr = CompileShader( "Code/Shader/Shader.hlsl", "GS_main", "gs_5_0", nullptr, &pgs ) ) )
    {
    ...
    if( SUCCEEDED( hr = CompileShader( "Code/Shader/Shader.hlsl", "PS_main", "ps_5_0", nullptr, &pps ) ) )
    {
...
HRESULT Scene::Initialize( ID3D11Device* device, ID3D11DeviceContext* deviceContext )
{
	...
	if( FAILED( hr = mParticleSystem.Initialize( device, deviceContext, XMFLOAT3( 0.0f, 2.0f, 0.0f ) ) ) )
		return hr;
